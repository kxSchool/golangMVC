...
// UserArticle is yes.
func UserArticle(userid int, page int, size int, db *sql.DB) (*models.Articles, error) {
...
func Detail(id string, db *sql.DB) (*models.Article, error) {
...
// Post is yes
func Post(article models.Article, db *sql.DB) (bool, error) {
...
func GetArticle(id string, db *sql.DB) (*models.Article, error) {
	var result models.Article
	body := ""
	err := db.QueryRow("select Id,PostType,Subject,Picture,Body,Origin,NickName,AddDate from Article where Id = ?", id).Scan(&result.ID, &result.PostType, &result.Subject, &result.Picture, &body, &result.Origin, &result.NickName, &result.AddDate)
	utils.CheckErr(err)
	result.Body = template.HTML(body)
	return &result, nil
}
...
// Update is yes
func Update(article models.Article, db *sql.DB) (bool, error) {
