...
func UserArticle(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	result, err := ado.UserArticle(id, page, 15, db)
	if err != nil {
		log.Fatal(err)
	}
...
func GetArticle(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	id := ps.ByName("id")
	result, err := ado.GetArticle(id, db)
	if err != nil {
		log.Fatal(err)
	}
...
func Post(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if adding {
		result, err := ado.Post(article, db)
		if err != nil {
			log.Fatal(err)
		}
		msg.Success = result
	} else {
		result, err := ado.Update(article, db)
		if err != nil {
			log.Fatal(err)
		}
		msg.Success = result
